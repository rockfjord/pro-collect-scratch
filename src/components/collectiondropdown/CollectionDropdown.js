...
const CollectionDropdown = React.memo((props) => {
  ...
  const initialized =
    state.collectionData !== null && state.collectionData !== undefined;
  ...
  const [collectionDropDownOptions, setCollectionDropDownOptions] = useState(
    collections
      ? createCollections(collections, false, "All Collections", true)
      : [getDefaultOption()]
  );

  const onSearchCollection = (e, collections) => {
    setCollectionDropDownOptions(
      createCollections(collections, e.target.value)
    );
  };
  ...
  const option =
    collection && collection !== "*"
      ? getCollectionOption(collectionDropDownOptions, collection)
      : -1;
        ...
        <div>
          ...
          >
            <Autocomplete
              ...
              defaultValue={
                option > -1 ? collectionDropDownOptions[option] : null
              }
              id="collection-box"
              style={{ width: "100%" }}
              popupIcon={null}
