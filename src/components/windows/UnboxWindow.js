...
function UnboxWindow(props) {
  ...
  const parsePacks = (data) => {
    ...
    for (let i = 0; i < packs.length; i++) {
      if (
        packs[i].templateId.toString() === asset.template.template_id.toString()
      ) {
        setPack(packs[i]);
      }
    }
  ...
  useEffect(() => {
    Promise.all([state.packData, state.templateData]).then((res) =>
      parsePacks(res)
    );
  }, []);
  ...
  const loadResults = (templateIds) => {
    if (templateIds && templateIds.length > 0) {
      ...
      if (
        pack.displayData &&
        pack.displayData.animation &&
        pack.displayData.animation.drawing
      ) {
