...
export default function MarketButtons(props) {
  ...
  const cancel = async () => {
    let { sale_id } = listing
      ? listing
      : asset.sales && asset.sales.length > 0 && asset.sales[0];
  ...
  const cancelAuction = async () => {
    let { auction_id } = listing
      ? listing
      : asset.auctions && asset.auctions.length > 0 && asset.auctions[0];
  ...
  const unbox = () => {
    ...
    dispatch({
      type: "SET_CALLBACK",
      payload: (unboxed) => handleUnboxed(unboxed)
    });
  ...
  const sell = async () => {
    ...
    dispatch({
      type: "SET_CALLBACK",
      payload: (sellInfo) => handleList(sellInfo)
    });
  ...
  const popError =
    state && state.error && state.error["asset_id"] === asset_id
      ? state.error["error"]
      : null;
  ...
  if (listing) {
    ...
    const infoField = (
      <Container>
        <div className="relative py-0 px-1 text-xs w-full">
          {formattedPrice}
        </div>
      </Container>
    );
    ...
    const auctioncancelable =
      userName && userName === seller && auction_id && !canceled;
    ...
    const cancelField = (
      <Container>
        <div
          className="relative text-center ml-auto mr-auto top-0 left-0"
          onClick={cancel}
        >
          Cancel
        </div>
    ...
    const claimable =
      userName && userName === unboxer && page === "unclaimed_packs";

    const sellable =
      userName &&
      (userName === owner ||
        (listing && listing["buyer"] && listing["buyer"] === userName)) &&
      (!listed || bought || canceled) &&
      !unpackable &&
      !claimable;
