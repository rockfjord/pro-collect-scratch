...
const StaticAssetList = (props) => {
  ...
  useEffect(() => {
    ...
    if (type === "listings") initListings(1, collection);
    if (type === "assets") initAssets(1, collection);
    if (type === "sales") initSales(1, collection);
  }, [type, collection]);
        ...
        >
          ...
          {assets && assets["success"]
            ? assets["data"].map((asset, index) => (
                <AssetCard {...props} index={index} assets={[asset]} />
              ))
            : ""}
          {sales && sales["success"]
